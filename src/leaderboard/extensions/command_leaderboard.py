...
@loader.command
class LeaderboardCommand(lightbulb.SlashCommand, name="leaderboard", description="Displays the guild's top 10 users by recorded message and attachment counts."):
    ...
    async def invoke(self, ctx: lightbulb.Context, db: aiosqlite.Connection) -> None:
        ...
        message_leaders, attachment_leaders = await database.get_leaderboard_for_guild(db, guild_id)
        
        container = hikari.impl.ContainerComponentBuilder(accent_color=config.DEFAULT_COLOUR)
        ...
        for rank, row in enumerate(message_leaders, start=1):
            container.add_text_display(f"{rank}. {row["user_username"]} - {row["message_count"]}")
        ...
        for rank, row in enumerate(attachment_leaders, start=1):
            container.add_text_display(f"{rank}. {row["user_username"]} - {row["attachment_count"]}")
